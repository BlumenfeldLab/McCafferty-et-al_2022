function [ Seizures, PSDparams, AnalysisParams] = FreeLickSzProperties( FilLoc, Epoch, WinSize, PreLickRates, ContBLRates, ContSZRates, BaseLine, FsEEG )
...
%ANIMALS_TO_ANALYZE = {'FL2\','FL6\','FL10\','FL20\','FL23\','FL24\','FL25\','FL26\'};%,'FL4\','FL5\','FL6\','FL7\','FL10\','FL12\','FL16\','FL17\','FL19\','FL20\','FL22\','FL23\','FL24\','FL25\','FL26\','FL27\'}; % Enter the name of the animals with a backslash
%ANIMALS_TO_ANALYZE = {'FL2\','FL4\','FL5\','FL6\','FL7\','FL10\','FL12\', 'FL16\', 'FL17\','FL19\','FL20\','FL21','FL22\','FL23\','FL24\','FL25\','FL26\','FL27\'}; % Enter the name of the animals with a backslash
%ANIMALS_TO_ANALYZE = {'FL4\'};%, 'FL29\', 'FL30\', 'FL31\', 'FL32\', 'FL33\', 'FL35\', 'FL36\' };
...
% These next parameters are for moving window spectrogram (mtspecgramc)
% calculations
params.tapers=[2,3];
params.fpass=[0,100];
params.Fs = FsEEG;
...
% And some early conversions from seconds to our sample units
FsBaseLine = BaseLine*FsEEG;
...
for s_animal = 1:size(ANIMALS_TO_ANALYZE,2) % go through each animal's folder
    ...
    cal = dir([FilLoc,(ANIMALS_TO_ANALYZE{1,s_animal}),'*_cali.mat']); % spike2/MEDPC synchronization
    ...
    for x_session = 1:size(allchans,1) % iterates through each recording in the animal folder
        ...
        if isempty(allseiz) == 0; % making sure there were some seizures in this session, if not there's no point wasting processing time
            ...
            for r_seizure = 2:size(SzTms,1)-1; % go through seizures, starting with 2nd as 1st is actually just an "end time"
                presztime = min([SzTms(r_seizure,1)-SzTms(r_seizure-1,2),FsBaseLine,]); % amount of baseline available to us before seizure
                prelickrate = sum(licklog(SzTms(r_seizure,1)-presztime:SzTms(r_seizure,1)))/presztime*FsEEG; % licks per second in this available baseline period
                if prelickrate >= PreLickRates(1,1) && prelickrate <= PreLickRates(1,2); % if this baseline licks/s was within the desired "control-like" range
                    SzCount = SzCount+1; % add one to the seizure counter
                    
                    % next we filter our lick logs to contain only licks
                    % from this presz and this sz period, all else being nan
                    ...
                    Seizures(SzCount).preszlicklog = licklog(SzTms(r_seizure,1)-(epochPaddingInSec*FsEEG):SzTms(r_seizure,1));
                    Seizures(SzCount).postszlicklog = licklog(SzTms(r_seizure,2):SzTms(r_seizure,2)+(epochPaddingInSec*FsEEG));
                    Seizures(SzCount).szlicklog = licklog(SzTms(r_seizure,1):SzTms(r_seizure,2));
                    ...
                    Seizures(SzCount).LicksStart(1,1:EpochWins) = sum(reshape(thispreszlicklog(max(SzTms(r_seizure,1)-EpochSamps,1):SzTms(r_seizure,1)-1),WinSamps,EpochSamps/WinSamps),1)/WinSize;
                    % and then the same for the start of seizure licks
                    Seizures(SzCount).LicksStart(1,EpochWins+1:EpochWins*2) = sum(reshape(thisszlicklog(SzTms(r_seizure,1):SzTms(r_seizure,1)+EpochSamps-1),WinSamps,EpochSamps/WinSamps),1)/WinSize;
                    
                    %% FOURTH OUTPUT: SEIZURE END LICKS IN BINS
                    % next for end of seizure licks
                    Seizures(SzCount).LicksEnd(1,1:EpochWins) = sum(reshape(thisszlicklog(SzTms(r_seizure,2)-EpochSamps:SzTms(r_seizure,2)-1),WinSamps,EpochSamps/WinSamps),1)/WinSize;
                    % and finally for post-seizure licks
                    Seizures(SzCount).LicksEnd(1,EpochWins+1:EpochWins*2) = sum(reshape(thispstszlicklog(SzTms(r_seizure,2):SzTms(r_seizure,2)+EpochSamps-1),WinSamps,EpochSamps/WinSamps),1)/WinSize;
                    
                    %% FIFTH OUTPUT: SEIZURE START EEG RAW DATA and vRMS
                    % modified by jacob prince to also export raw data
                    % because we may want to visualize raw seizures or
                    % extract new features to classify seizure severity
                    
%                     thispreszeeg = nan(size(allEEG));thispreszeeg(SzTms(r_seizure-1,2):SzTms(r_seizure,1)-1) = allEEG(SzTms(r_seizure-1,2):SzTms(r_seizure,1)-1);
%                     % creates a variable the size of the entire session but
%                     % with only the elements during this single preseizure 
%                     thisszeeg = nan(size(allEEG)); thisszeeg(SzTms(r_seizure,1):SzTms(r_seizure,2)) = allEEG(SzTms(r_seizure,1):SzTms(r_seizure,2));
%                     thispstszeeg = nan(size(allEEG));thispstszeeg(SzTms(r_seizure,2):SzTms(r_seizure+1,1)) = allEEG(SzTms(r_seizure,2):SzTms(r_seizure+1,1));
%                     
%                     % also add raw eeg data from sz period and +/- a few seconds to the Seizure struct
%                     % added by jacob prince 3/4/19
%                     Seizures(SzCount).preszeeg = allEEG(SzTms(r_seizure,1)-(epochPaddingInSec*FsEEG):SzTms(r_seizure,1));
%                     Seizures(SzCount).postszeeg = allEEG(SzTms(r_seizure,2):SzTms(r_seizure,2)+(epochPaddingInSec*FsEEG));
%                     Seizures(SzCount).szeeg = allEEG(SzTms(r_seizure,1):SzTms(r_seizure,2));
%                     
%                     Seizures(SzCount).vRMSStart(1,1:EpochWins) = WINrms(thispreszeeg(max(SzTms(r_seizure,1)-EpochSamps,1):SzTms(r_seizure,1)-1),WinSamps,0,1);
%                     Seizures(SzCount).vRMSStart(1,EpochWins+1:EpochWins*2) = WINrms(thisszeeg(SzTms(r_seizure,1):SzTms(r_seizure,1)+EpochSamps-1),WinSamps,0,1);
%                     
%                     %% SIXTH OUTPUT: SEIZURE END EEG vRMS
%                     Seizures(SzCount).vRMSEnd(1,1:EpochWins) = WINrms(thisszeeg(SzTms(r_seizure,2)-EpochSamps:SzTms(r_seizure,2)-1),WinSamps,0,1);
%                     Seizures(SzCount).vRMSEnd(1,EpochWins+1:EpochWins*2) = WINrms(thispstszeeg(SzTms(r_seizure,2):SzTms(r_seizure,2)+EpochSamps-1),WinSamps,0,1);
%                     
%                     %% SEVENTH OUTPUT: SEIZURE START SPECTROGRAM
%                       [s,t,f] = mtspecgramc(thispreszeeg(max(SzTms(r_seizure,1)-EpochSamps,1):SzTms(r_seizure,1)-1),movingwin,params); % output the frequencies and PSD of the signal, with a 0.5s window and no overlap at 1000 frequencies at 1000 Hz
% %                     [s,f,t] = spectrogram(thispreszeeg(max(SzTms(r_seizure,1)-EpochSamps,1):SzTms(r_seizure,1)-1),WinSamps,0,[1:1:100],1000);
%                     Seizures(SzCount).SpecStart(:,1:EpochWins-1) = s';
%                       [s,t,f] = mtspecgramc(thisszeeg(SzTms(r_seizure,1):SzTms(r_seizure,1)+EpochSamps-1),movingwin,params);
% %                     [s,f,t] = spectrogram(thisszeeg(SzTms(r_seizure,1):SzTms(r_seizure,1)+EpochSamps-1),WinSamps,0,[1:1:100],1000);
%                     Seizures(SzCount).SpecStart(:,EpochWins:(EpochWins-1)*2) = s';
%                     
%                     %% EIGHTH OUTPUT: SEIZURE END SPECTROGRAM
%                       [s,t,f] = mtspecgramc(thisszeeg(SzTms(r_seizure,2)-EpochSamps:SzTms(r_seizure,2)-1),movingwin,params); % output the frequencies and PSD of the signal, with a 0.5s window and no overlap at 1000 frequencies at 1000 Hz
% %                     [s,f,t] = spectrogram(thisszeeg(SzTms(r_seizure,2)-EpochSamps:SzTms(r_seizure,2)-1),WinSamps,0,[1:1:100],1000);
%                     Seizures(SzCount).SpecEnd(:,1:EpochWins-1) = s';
%                       [s,t,f] = mtspecgramc(thispstszeeg(SzTms(r_seizure,2):SzTms(r_seizure,2)+EpochSamps-1),movingwin,params);
% %                     [s,f,t] = spectrogram(thispstszeeg(SzTms(r_seizure,2):SzTms(r_seizure,2)+EpochSamps-1),WinSamps,0,[1:1:100],1000);
%                     Seizures(SzCount).SpecEnd(:,EpochWins:(EpochWins-1)*2) = s';
%                     
            ...
            % After extracting desired info from appropriate seizures, we
            % need to find control periods that have similar "pre" lick
            % rates
            
%             a = 1;b = size(szlog,1)-EpochSamps*3;
%             randomrange1 = (b-a).*rand(1000,1) + a; 
%             % selecting 10000 random values that are between the start of the recording and 3 epochs from the end (one epoch for pre, one for sz-matched, one for post-sz)
%             
%             % next we will select those of these values that 1) don't include
%             % any seizures and 2) are similar to the pre-seiz baseline
%             
%             SCCount = 0;ECCount = 0;
%             for c = 1:size(randomrange1,1)
%                 if sum(szlog(randomrange1(c,1):randomrange1(c,1)+EpochSamps*2,1)) == 0 ... % if there are no seiz in the range, and (next lines) the lick rate roughly matches the pattern seen before seizures start
%                         && sum(licklog(randomrange1(c,1)+EpochSamps-(10*FsEEG):randomrange1(c,1)+EpochSamps-(7.5*FsEEG),1))/2.5 >= ContBLRates(1,1) ... % and licks/sec in first quarter of baseline are between this value...
%                         && sum(licklog(randomrange1(c,1)+EpochSamps-(10*FsEEG):randomrange1(c,1)+EpochSamps-(7.5*FsEEG),1))/2.5 <= ContBLRates(2,1) ... % and this value
%                         && sum(licklog(randomrange1(c,1)+EpochSamps-(7.5*FsEEG):randomrange1(c,1)+EpochSamps-(5*FsEEG),1))/2.5 >= ContBLRates(1,2) ... % and licks/sec in second quarter of baseline are between this value...
%                         && sum(licklog(randomrange1(c,1)+EpochSamps-(7.5*FsEEG):randomrange1(c,1)+EpochSamps-(5*FsEEG),1))/2.5 <= ContBLRates(2,2) ... % and this value
%                         && sum(licklog(randomrange1(c,1)+EpochSamps-(5*FsEEG):randomrange1(c,1)+EpochSamps-(2.5*FsEEG),1))/2.5 >= ContBLRates(1,3) ... % and licks/sec in third quarter of baseline are between this value...
%                         && sum(licklog(randomrange1(c,1)+EpochSamps-(5*FsEEG):randomrange1(c,1)+EpochSamps-(2.5*FsEEG),1))/2.5 <= ContBLRates(2,3) ... % and this value
%                         && sum(licklog(randomrange1(c,1)+EpochSamps-(2.5*FsEEG):randomrange1(c,1)+EpochSamps,1))/2.5 >= ContBLRates(1,4) ... % and in fourth quarter of baseline between this value
%                         && sum(licklog(randomrange1(c,1)+EpochSamps-(2.5*FsEEG):randomrange1(c,1)+EpochSamps,1))/2.5 <= ContBLRates(2,4) % and this value
%                         
%                     SCCount = SCCount+1; % then increment this tracker
%                     
%                     StrtControls(SCCount).LicksStart = nansum(reshape(licklog(randomrange1(c,1):randomrange1(c,1)+(EpochSamps*2)-1,1),WinSamps,EpochSamps*2/WinSamps),1)/WinSize; % and set the next row of control start licks to the pre and sz-matched parts
%                     StrtControls(SCCount).animal = ANIMALS_TO_ANALYZE{1,s_animal};
%                     StrtControls(SCCount).session = x_session;
% 
%                 elseif sum(szlog(randomrange1(c,1):randomrange1(c,1)+EpochSamps*2,1)) == 0 ... % if there are no seiz in the range, and (next lines) the lick rate roughly matches the pattern seen before seizures end
%                         && sum(licklog(randomrange1(c,1)+EpochSamps-(10*FsEEG):randomrange1(c,1)+EpochSamps-(5*FsEEG),1))/2.5 >= ContSZRates(1,1) ... % and licks/sec in first half of end of seizure are between this value...
%                         && sum(licklog(randomrange1(c,1)+EpochSamps-(10*FsEEG):randomrange1(c,1)+EpochSamps-(5*FsEEG),1))/2.5 <= ContSZRates(2,1) ... % and this value
%                         && sum(licklog(randomrange1(c,1)+EpochSamps-(5*FsEEG):randomrange1(c,1)+EpochSamps,1))/2.5 >= ContSZRates(1,2) ... % and licks/sec in second half of end of seizure are between this value...
%                         && sum(licklog(randomrange1(c,1)+EpochSamps-(5*FsEEG):randomrange1(c,1)+EpochSamps,1))/2.5 <= ContSZRates(2,2) % and this value
%                     
%                     ECCount = ECCount+1;
%                     
%                     EndControls(ECCount).LicksEnd = nansum(reshape(licklog(randomrange1(c,1):randomrange1(c,1)+(EpochSamps*2)-1,1),WinSamps,EpochSamps*2/WinSamps),1)/WinSize; % then set the next row of control end licks to the seizure end and post sz parts
%                     EndControls(ECCount).animal = ANIMALS_TO_ANALYZE{1,s_animal};
%                     EndControls(ECCount).session = x_session;
%                         
%                 end
%             end
...
AnalysisParams.FilLoc = FilLoc;
AnalysisParams.Epoch = Epoch;
AnalysisParams.WinSize = WinSize;
AnalysisParams.PreLickRates = PreLickRates;
AnalysisParams.ContBLRates = ContBLRates;
AnalysisParams.ContSZRates = ContSZRates;
AnalysisParams.BaseLine = BaseLine;
AnalysisParams.FsEEG = FsEEG;
